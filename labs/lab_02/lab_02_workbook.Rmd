...
```{r}
acros_md_baltimorecity <- read_csv("data/arcos-md-baltimore-city-24510-pharmacy.csv")
```
...
```{r}
glimpse(acros_md_baltimorecity)
```
...
arcos_md_baltimorecity_pharmacies <- acros_md_baltimorecity %>%
  select(buyer_name)

arcos_md_baltimorecity_pharmacies
```
...
arcos_md_baltimorecity_trimmed <- acros_md_baltimorecity %>%
  select(buyer_name, buyer_dea_no, buyer_city, total_dosage_unit, total_records)
...
#Answer: Newcare Home Health Services with 6133600 total pills
...
arcos_md_baltimore_sorted_trimmed <- arcos_md_baltimore %>%
  select(buyer_dea_no, buyer_name, buyer_city, total_dosage_unit, total_records) %>%
  arrange(desc(total_records))

arcos_md_baltimore_sorted_trimmed

#Answer: Giant Pharmacy 166, 10365 total shipments
```
...
#Answer: Oshinshky, Alan E MD and Jakobovits, Julian MD both got 4 number of pills. These are doctors instead of pharmacies. 


#Answer: NIA RESEARCH PHARMACY, 100 pills
```
