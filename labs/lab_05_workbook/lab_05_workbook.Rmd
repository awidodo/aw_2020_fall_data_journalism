...
```{r}
covid_county_total_cases <- covid_county %>%
  select(cases) %>%
  sum()

covid_county_total_cases
```
* count the number of values, with n()
```{r}
covid_county_values <- covid_county %>%
  select(cases) %>%
  summarise(count=n())

covid_county_values

#3123 values for cases
```

* find the smallest value, with min()
* find the largest value, with max()
```{r}
covid_county_min <- covid_county %>%
 group_by(state) %>%
...
#Smallest value with min() is 3 for Wyoming, and 1 for individual counties

covid_county_max <- covid_county %>%
 group_by(county) %>%
  summarise(max_cases = max(cases)) %>%
  arrange(desc(max_cases))

covid_county_max
#Largest value with max() is 240749 for California, and 240749 for Los Angeles
```
* find average values, with mean()
```{r}
covid_county_mean <- covid_county %>%
 group_by(state) %>%
...
us_county_average_covid_deaths <- covid_county %>% 
  summarise(average_total_deaths=mean(deaths))
...
smallest_number_counties <- covid_county %>%
  group_by(state) %>%
  summarise(total_counties = n()) %>%
  arrange(total_counties)

smallest_number_counties
...
```{r}
#Q:What is the average number of covid deaths in Maryland? What county has the most and least?

state_deaths_in_maryland<- covid_county %>%
  group_by(county) %>%
  filter(state =="Maryland") %>%
  summarise(deaths_in_average_county = mean(deaths)) %>%
  arrange(desc(deaths_in_average_county))

state_deaths_in_maryland

#A: Montgomery had 820 deaths, Garrett County had 0 deaths
```
