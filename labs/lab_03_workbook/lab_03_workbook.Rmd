...
# A: 2 areas include Hancock, Georgia and Galax city, Virginia
...
test_filter <- covid_county_monthly_case_rate %>%
  filter(str_detect(county, "City|city"))
...
covid_maryland_june_aug_pct_change <- covid_county_monthly_death_rate %>%
  select(county, state, d_2020_06_30, d_2020_08_30) %>%
  filter(state == "Maryland") %>%
  mutate(covid_maryland_june_aug_pct_change = (d_2020_08_30 - d_2020_06_30)/d_2020_06_30) %>%
  arrange(desc(covid_maryland_june_aug_pct_change))

# Write the name of the object you just created below so it displays below in the markdown document

covid_maryland_june_aug_pct_change
...
covid_texas_may_aug_pct_change <- covid_county_monthly_case_rate %>%
  ...
  filter(state == "Texas") %>%
  mutate(covid_texas_may_aug_pct_change = (d_2020_08_30 - d_2020_05_30)/d_2020_05_30) %>%
  arrange(covid_texas_may_aug_pct_change)

covid_texas_may_aug_pct_change
...
Which county that has city in its name have the highest percentage change of cumalative death rates between March to August?

```{r}
covid_city_march_aug_pct_change <- covid_county_monthly_death_rate %>%
  select(county, state, d_2020_03_30, d_2020_08_30) %>%
  filter(str_detect(county,"City|city")) %>%
  mutate(covid_city_march_aug_pct_change = (d_2020_08_30 - d_2020_03_30)/d_2020_03_30) %>%
  arrange(desc(covid_city_march_aug_pct_change))

covid_city_march_aug_pct_change

#Answer: Baltimore city, Maryland has the highest percentage change in death rates between March to August
