...
arcos_wv_mingo_trimmed <- arcos_wv_mingo %>%
  select(buyer_dea_no)
#-- the word buyer was spelt without the y, but now the code runs 
```
...
```{r}
arcos_md_baltimore <- read_csv("data/arcos-md-baltimore-city-24510-pharmacy.csv")
```
...
```
```{r}
#selecting columns in the order you want it to be in the table
arcos_md_baltimore_trimmed <- arcos_md_baltimore %>%
  select(buyer_dea_no, buyer_name, buyer_city, total_dosage_unit, total_records)

#sorting the total shipment
arcos_md_baltimore_sorted <- arcos_md_baltimore_trimmed %>%
  arrange(desc(total_records))

#sorting the total pills
arcos_md_baltimoreo_sorted <- arcos_md_baltimore_trimmed %>%
  arrange(desc(total_dosage_unit))

#print the sorted table
arcos_md_baltimore_sorted
```
Most total shipments: Newcare Home Health Services
Most pills: Giant Pharmacy 166 (Used environment tab to open table and sort the data to find this one)
