...
#Because the data is dirty it won't work. Column name is correct. Fix put it in tik marks.` `
```
...
county_covid_deaths <- county_covid_deaths_dirty %>%
  clean_names()
#clean_names janitor package takes all the columns names and looks to see if there's special characters and removes them
...
#converting column: character to a numerical value  
county_covid_deaths
```
Let's try answering our question again.
...
#replacing na means that it equals 0
county_covid_deaths
```
Let's try answering our question again.
